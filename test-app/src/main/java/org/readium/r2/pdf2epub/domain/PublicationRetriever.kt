...
import org.readium.r2.pdf2epub.utils.copyToNewFile
import org.readium.r2.pdf2epub.utils.extensions.copyToTempFile
import org.readium.r2.pdf2epub.utils.extensions.moveTo
...
class PublicationRetriever(
    ...
    suspend fun retrieveFromStorage(
        ...
        var localResult = localPublicationRetriever
            .retrieve(uri)
            .getOrElse { return Try.failure(it) }
        println(localResult.format?.mediaType.toString())


        println(localResult.tempFile.name)
        println(localResult.coverUrl)

        println(localResult.format?.mediaType)
        println(localResult.format?.mediaType?.type)
    ...
    println("final result")
        println(finalResult.publication)
        println(finalResult.coverUrl)
    ...
    private suspend fun retrieveFromStorage(
        ...
        if (
            ...
            return if (lcpService == null) {
                ...
                println("return mid")
                println(acquisition.localFile.path)
                println(coverUrl)

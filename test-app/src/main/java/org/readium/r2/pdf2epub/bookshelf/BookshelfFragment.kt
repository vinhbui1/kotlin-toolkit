...
import org.readium.r2.pdf2epub.Application
import org.readium.r2.pdf2epub.R
import org.readium.r2.pdf2epub.data.model.Book
import org.readium.r2.pdf2epub.databinding.FragmentBookshelfBinding
import org.readium.r2.pdf2epub.opds.GridAutoFitLayoutManager
...
class BookshelfFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        bookshelfAdapter.registerAdapterDataObserver(object : RecyclerView.AdapterDataObserver() {
            override fun onChanged() {
                super.onChanged()
                updateEmptyState() // Call updateEmptyState() when data changes
            }

            override fun onItemRangeInserted(positionStart: Int, itemCount: Int) {
                super.onItemRangeInserted(positionStart, itemCount)
                updateEmptyState() // Call when items are inserted
            }

            override fun onItemRangeRemoved(positionStart: Int, itemCount: Int) {
                super.onItemRangeRemoved(positionStart, itemCount)
                updateEmptyState() // Call when items are removed
            }
        })
        ...
        val mimeTypes = arrayOf("application/pdf", "application/epub+zip", "audio/mpeg", "audio/x-m4a", "audio/x-audiobook")
        binding.bookshelfAddBookFab.setOnClickListener {
            ...
            MaterialAlertDialogBuilder(requireContext())
                ...
                .setPositiveButton(getString(R.string.ok)) { _, _ ->
                    when (selected) {
                        0 -> appStoragePickerLauncher.launch("application/pdf")
                        1 -> sharedStoragePickerLauncher.launch(mimeTypes)
    ...
    private fun updateEmptyState() {
        if (bookshelfAdapter.itemCount == 0) {
            binding.emptyStateLayout.visibility = View.VISIBLE
            binding.bookshelfBookList.visibility = View.GONE
        } else {
            binding.emptyStateLayout.visibility = View.GONE
            binding.bookshelfBookList.visibility = View.VISIBLE
        }
